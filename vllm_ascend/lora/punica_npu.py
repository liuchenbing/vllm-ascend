...
class PunicaWrapperNPU(PunicaWrapperBase):
    ...
    def _apply_shrink_fused(
        ...
        # Reference: SGLang merges (num_loras, input_dim, rank) * N -> (num_loras, input_dim, total_rank)
        if num_slices > 1:
            # Check if all slices have the same rank and input_dim
            first_shape = lora_a_stacked[0].shape
            if len(first_shape) != 3:
                # Not the expected 3D tensor, fallback to sequential
                pass
            else:
                first_rank = first_shape[-1]
                first_input_dim = first_shape[-2]
                
                # Verify all slices have the same shape
                all_same_shape = all(
                ...
                if all_same_shape and first_rank > 0:
                    ...
                    # Create a temporary merged buffer for shrink output
                    # Shape: (batch_size, total_rank)
                    merged_buffer = torch.zeros(
                        batch_size, total_rank,
                        dtype=y[0].dtype,
                        device=y[0].device
                    )
                    merged_buffer_flat = merged_buffer.view(-1, total_rank)
                    ...
                    # Split merged buffer back to individual slices
                    for slice_idx in range(num_slices):
                        y_slice = y[slice_idx]
                        y_org = y_slice
                        y_flat = y_slice.view(-1, rank_per_slice)
                        # Copy from merged buffer to individual slice buffer
